import Game


    ##########################################################
    #Testing list_possible_gobblets
    ##########################################################

    #Since the index of the player is initialized by 0 and we are at the start of the game,
    #we suppose that the list of gobblets will include 3 gobblets one at top of each stack
"""
def main():
    game_instance = Game.Game()

    possible_gobblets_list = []
    possible_gobblets_list.extend(game_instance.list_possible_gobblets())
    print("Done") 
"""

    ##########################################################
    #Testing list_possible_moves
    ##########################################################

    #Since we are at the start of the game and the board is empty,
    #we suppose that all indexes of positions on board will be returned
"""
def main():
    game_instance = Game.Game()

    possible_moves_list = []
    possible_moves_list.extend(game_instance.list_possible_moves(game_instance.FirstPlayerGobbletsArray[0]))
    print("Done")
"""
    ##########################################################
    #Testing all_valid_moves
    ##########################################################

    #Since we are at the start of the game we expect 48 valid moves
    #3 gobblets (the ones on top of each stack) each has 16 available moves
'''
def main():
    game_instance = Game.Game()

    valid_moves = []
    valid_moves.extend(game_instance.all_valid_moves())
    print("Done")
'''

    ##########################################################
    #Testing make_move
    ##########################################################

#Scenario 1

'''
def main():
    game_instance = Game.Game()

    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[2],game_instance.BoardItemsArray[2])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[3],game_instance.BoardItemsArray[3])

    print(game_instance.GameState)
'''


#Scenario 2

'''
def main():
    game_instance = Game.Game()

    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[2])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[3])


    print(game_instance.GameState)
'''
#######################################
# New Test Scenarios                  #
#######################################
#Scenario 3

# Testing Gobbling up [Positive Scenario]
# Gobbeling up externally

'''
def main():
    game_instance = Game.Game()

    game_instance.CurrentPlayerIndex = 0
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.CurrentPlayerIndex = 0
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])
    game_instance.CurrentPlayerIndex = 0
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[2],game_instance.BoardItemsArray[2])

    game_instance.CurrentPlayerIndex = 1
    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[0],game_instance.BoardItemsArray[2])

    print(game_instance.GameState)
'''


#Scenario 4

# Testing Gobbling up [Negative Scenario]
# Player 2 Should not be able to gobble up in case player 1 doesn't have 3 gobblets in a row(external move)
'''
def main():
    game_instance = Game.Game()

    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[2],game_instance.BoardItemsArray[3])

    game_instance.CurrentPlayerIndex = 1
    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[0],game_instance.BoardItemsArray[3])

    print(game_instance.GameState)
'''

#Scenario 5

# Testing Gobbling up [Negative Scenario]
# Gobblet cannot gobbel up gobblet of the same size(internal move)
'''
def main():
    game_instance = Game.Game()

    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])

    game_instance.CurrentPlayerIndex = 1
    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[0],game_instance.BoardItemsArray[2])
    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])

    print(game_instance.GameState)
'''
#Scenario 6

# Testing Gobbling up [Negative Scenario]
# Gobblet cannot gobbel up gobblet of bigger size(internal move)
'''
def main():
    game_instance = Game.Game()

    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])

    game_instance.alternate_player()

    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[1],game_instance.BoardItemsArray[2])
    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[1],game_instance.BoardItemsArray[0])

    print(game_instance.GameState)
'''
#Scenario 7

# Testing Gobbling up [Positive Scenario]
# Larger Gobblet can gobbel up smaller gobblet(internal move)

def main():
    game_instance = Game.Game()

    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[0],game_instance.BoardItemsArray[0])
    game_instance.make_move(game_instance.FirstPlayerGobbletsArray[1],game_instance.BoardItemsArray[1])

    game_instance.CurrentPlayerIndex = 1

    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[0],game_instance.BoardItemsArray[4])
    game_instance.make_move(game_instance.SecondPlayerGobbletsArray[0],game_instance.BoardItemsArray[1])

    print(game_instance.GameState)



if __name__ == "__main__":
    main()


